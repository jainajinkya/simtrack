...
{
  if (_num_sift[0]*_num_sift[1] > 134217728) {
    std::cout << "SiftMatchCU::GetBestMatch: trying to match " << _num_sift[0] << " with " << _num_sift[1]
              << " keypoints exceeds 1D (linear) texture limit!" << std::endl;
    return 0;
  }
